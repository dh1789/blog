...
**이 섹션은 컨텍스트 압축, 세션 재개, 요약 후에도 반드시 참조해야 합니다.**

### 컨텍스트 압축 시 필수 행동

```
컨텍스트 압축/요약 감지 시:
1. 이 SKILL.md 파일을 반드시 다시 읽기
2. 아래 체크리스트 확인 후 작업 진행
3. CLAUDE.md의 절대 규칙 재확인
```

### 블로그 작업 감지 키워드

다음 키워드 감지 시 이 스킬을 **반드시** 참조:
...
블로그 관련 작업 시작 전 **반드시** 확인:

| # | 항목 | 확인 |
|---|------|------|
| 1 | 대상 파일 경로 확인 (`content/posts/ko/` 또는 `content/posts/en/`) | ☐ |
| 2 | frontmatter 필수 필드 존재 (title, slug, excerpt, tags, language) | ☐ |
| 3 | 코드 블록이 표준 마크다운 형식 (` ``` `) 사용 | ☐ |
| 4 | SyntaxHighlighter/WordPress shortcode 미사용 | ☐ |
| 5 | 이미지 경로가 상대 경로로 올바르게 설정 | ☐ |
...
| # | 항목 | 확인 |
|---|------|------|
| 1 | WordPress 포스트 ID 확인 및 보고 | ☐ |
| 2 | 발행 URL 확인 | ☐ |
| 3 | 자동 번역 성공/실패 여부 확인 | ☐ |
| 4 | 번역 실패 시: 8단계 검증 결과 보고 | ☐ |
| 5 | Polylang 언어 연결 완료 확인 | ☐ |

### 번역 완료 후

| # | 항목 | 확인 |
|---|------|------|
| 1 | 코드 블록 수 일치 (원본 vs 번역) | ☐ |
| 2 | 링크 수 일치 | ☐ |
| 3 | 이미지 경로 보존 | ☐ |
| 4 | 라인 수 차이 ±20% 이내 | ☐ |
| 5 | 영문 파일 `content/posts/en/`에 저장 | ☐ |
...
```
❌ 사용자 승인 없이 publish 명령 실행
❌ SEO 70점 미만 상태로 발행
❌ SyntaxHighlighter/WordPress shortcode 사용
❌ 체크리스트 미확인 상태로 작업 진행
❌ 번역 검증 실패 무시하고 발행
```
