...
**⚠️ 이 섹션은 발행 요청 시 가장 먼저 확인해야 합니다.**

### 발행 요청 시 필수 출력 순서

| 순서 | 우선순위 | 출력 내용 | 템플릿 위치 |
|------|----------|----------|-------------|
| 1 | **MUST** | 📋 11단계 파이프라인 다이어그램 | Claude UX 지침 §1 |
| 2 | **MUST** | 🔧 자동 적용될 기능 테이블 (7개 항목) | Claude UX 지침 §1 |
| 3 | **MUST** | ✅ Pre-flight 체크리스트 결과 | 실행 전 체크리스트 |
| 4 | **MUST** | 📊 SEO 분석 결과 테이블 | Claude UX 지침 §3 |
| 5 | **MUST** | ❓ "발행해도 될까요?" 승인 질문 | 실행 전 체크리스트 |

### 발행 완료 후 필수 출력 순서

| 순서 | 우선순위 | 출력 내용 | 템플릿 위치 |
|------|----------|----------|-------------|
| 1 | **MUST** | ✅ 각 단계별 완료 상태 보고 | Claude UX 지침 §2 |
| 2 | **MUST** | 🔍 번역 품질 검증 결과 (8단계) | Claude UX 지침 §4 |
| 3 | **MUST** | 🎉 최종 결과 보고 (ID, URL, Polylang) | Claude UX 지침 §5 |
| 4 | **MUST** | ❌ 에러 시: 실패 단계 + 복구 방법 | Claude UX 지침 §6 |
...
발행 전 반드시 보여줄 것:
□ 파이프라인 다이어그램 (11단계)
□ 자동 적용 기능 테이블 (광고, 시리즈, GitHub 등 7개)
□ Pre-flight 체크리스트 (5+4=9개 항목)
□ SEO 결과 테이블 (총점 + 항목별)
□ "발행해도 될까요?" 질문
...
□ 단계별 완료 상태 (✅/❌)
□ 번역 검증 결과 테이블
□ 최종 결과 (한글 ID/URL, 영문 ID/URL, Polylang)
```

---

## 📝 문서 작성 전 경험 수집 (Experience Interview) - MUST

**⚠️ 새 포스트를 작성할 때 반드시 사용자에게 경험을 먼저 질문해야 합니다.**

### 적용 시점

| 상황 | 경험 인터뷰 필요 여부 |
|------|---------------------|
| 새 포스트 작성 요청 | **MUST** - 반드시 질문 |
| 기존 포스트 발행 요청 | 해당 없음 (이미 작성됨) |
| 초안 수정 요청 | **SHOULD** - 경험담 부족 시 질문 |

### 경험 인터뷰 질문 (4가지 카테고리)

| 카테고리 | 질문 예시 | 답변 삽입 위치 |
|----------|----------|---------------|
| **motivation** | "이 기술을 배우게 된 계기가 무엇인가요?" | 서론/도입부 |
| **challenge** | "개발 중 가장 어려웠던 점은 무엇이었나요?" | 기술 설명 섹션 |
| **application** | "실제로 어디에 적용했거나 적용할 계획인가요?" | 실습/예제 섹션 |
| **insight** | "다른 사람들에게 전하고 싶은 인사이트는?" | 결론/마무리 |
...
다음 질문에 간단히 답변해주세요 (2-3문장):

1. 🎯 동기: 이 기술을 배우게 된 계기나 이유는?
2. 💪 도전: 학습/개발 중 가장 어려웠던 점은?
3. 🛠️ 적용: 실제로 어디에 적용했거나 할 계획인가요?
4. 💡 인사이트: 다른 분들에게 전하고 싶은 팁이 있나요?

(답변하기 어려운 항목은 "패스"라고 해주세요)
```

### 경험 인터뷰 프로세스

1. **[MUST]** 주제 확정 후 위 4가지 질문 템플릿 제시
2. **[MUST]** 사용자 답변을 바탕으로 자연스러운 경험담 문단 생성
3. **[MUST]** 초안 작성 시 경험담을 적절한 위치에 삽입
4. **[SHOULD]** 경험담이 문맥과 자연스럽게 연결되도록 작성
...
> 💡 저는 회사에서 검색 기능 개선 프로젝트를 맡으면서 이 기술을 처음 접하게 되었습니다.
> 기존 키워드 검색으로는 사용자 의도를 파악하기 어려웠는데, 시맨틱 검색을 도입하면서
> 검색 품질이 크게 향상되었습니다.

[다음 내용...]
```

### 빠뜨리면 안 되는 항목 요약 (작성 시)
...
| 기능 | 설명 | 적용 |
|------|------|------|
| 광고 삽입 | AdSense 코드 자동 삽입 | ✅ |
| SyntaxHighlighter | 코드 블록 변환 | ✅ |
| 시리즈 네비게이션 | 이전/다음 링크 생성 | ✅ |
| GitHub 링크 | TL;DR 뒤 저장소 링크 | ✅ |
| 번역 배너 | 영문 포스트에 원본 링크 | ✅ |
| Polylang 연결 | 한/영 양방향 연결 | ✅ |
| 이미지 업로드 | 미디어 라이브러리 업로드 | ⏭️ 해당없음 |
```
...
|------|----------|
| 광고 삽입 | 항상 (설정에 따라) |
| SyntaxHighlighter | 코드 블록 존재 시 |
| 시리즈 네비게이션 | 파일명 패턴 `{series}-day{N}` 감지 시 |
| GitHub 링크 | `docs/{series}-series-plan.md`에 GitHub URL 존재 시 |
| 번역 배너 | 영문 포스트 발행 시 |
| Polylang 연결 | 한/영 양쪽 발행 완료 시 |
| 이미지 업로드 | `--upload-images` 옵션 또는 로컬 이미지 경로 존재 시 |
